...
firebase.database().ref('online').on('value', (snapshot) => {
  ...
  for (const member of Object.keys(online)) {
    onlineDiv.innerHTML += `<div onclick="window.open('https://vk.com/id${member}', '_blank')" class="member">
...
firebase.database().ref('admins').on('value', (snapshot) => {
  ...
  for (const member of Object.keys(admins)) {
    adminsDiv.innerHTML += `<div onclick="window.open('https://vk.com/id${member}', '_blank')" class="member">
